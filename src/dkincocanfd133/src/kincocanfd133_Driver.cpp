...
// ?? <write only> in p161 of FD2(s) manual.
...
//驱动器温度 can't determined at present
...
//设备名称 can't find any explanation in reference manual. by ssj.
...
//硬件序列号 can't find any explanation in reference manual
...
std::string kincors232Fd133Driver::Cmd_SetEnable()
{
    ...
    msg.set_data_m_param3(0x000f);          // motor power on
    msg.set_data_m_param4(0);
    std::string res = msg.to_string();
    print_hex(res);
...
// motor power off
std::string kincors232Fd133Driver::Cmd_SetRelease()
{
	#if DEBUG_ECHO
    ...
    msg.set_data_m_param3(0x0006);              // motor power off
    msg.set_data_m_param4(0);
    std::string res = msg.to_string();
...
//driver working mode setup
std::string kincors232Fd133Driver::Cmd_SetMode(DRIVER_MODE mode)
{
    message msg;
    ...
    unsigned int data = SetCtrlMode;
    unsigned char *p = (unsigned char *)&data;
    msg.set_data_m_byte(*(p+1),1);
    msg.set_data_m_byte(*(p+2),2);
    msg.set_data_m_byte(*(p+0),3);

    msg.set_data_m_param3( (UINT16)mode );
    msg.set_data_m_param4(0);
...
std::string kincors232Fd133Driver::Cmd_SetAVelocity(UINT32 AVelocity)
{
    message msg;
    ...
    //msg.Set_CMD(SetVelocity);
    unsigned int data = SetAVelocity;
    unsigned char *p = (unsigned char *)&data;
    msg.set_data_m_byte(*(p+1),1);
    msg.set_data_m_byte(*(p+2),2);
    msg.set_data_m_byte(*(p+0),3);

    msg.set_data_m_param34((UINT32) ((AVelocity*65536.0*10000.0)/1000.0/4000.0) );
...
std::string kincors232Fd133Driver::Cmd_SetDVelocity(UINT32 DVelocity)
{
    message msg;
    ...
    msg.set_data_m_byte(WBYTE1, 0);
    //msg.Set_CMD(SetVelocity);
    unsigned int data = SetDVelocity;
    unsigned char *p = (unsigned char *)&data;
    msg.set_data_m_byte(*(p+1),1);
    msg.set_data_m_byte(*(p+2),2);
    msg.set_data_m_byte(*(p+0),3);

    msg.set_data_m_param34((UINT32) ((DVelocity*65536.0*10000.0)/1000.0/4000.0) );
...
std::string kincors232Fd133Driver::Cmd_SetStop()
{
    ...
    msg.set_data_m_param34((UINT32)(0*2731)*dir());     // velocity is zero
    //msg.set_data_m_param4(0);
    std::string res = msg.to_string();
    print_hex(res);
...
std::string kincors232Fd133Driver::Cmd_SetClearError()
{
    ...
    msg.set_data_m_param3(0x0086);          // why is the value ??? 0x80 p160 in FD2(S) manual
    msg.set_data_m_param4(0);
    std::string res = msg.to_string();
    print_hex(res);
...
//'电机当前位置
int kincors232Fd133Driver::CruPos()
{
    return m_ActPos;
}

//电机当前运行速度
short int kincors232Fd133Driver::cru_speed()
{
    return m_ActVelocity;
}

//'电机速度调节指令值
short int kincors232Fd133Driver::cru_speedcmd()
{
    return m_SetVelocity;
}

short int kincors232Fd133Driver::cru_driver_mode()
{
    return m_ctrlmode;
}

//母线电压值
short int kincors232Fd133Driver::Udc()
{
    return m_ActUdc;
}

//'电机有效转矩率(绝对值): 电流法
short int kincors232Fd133Driver::effectiveTorque_I()
{
    return m_ActCurrent;
}

//'功率器件温度（单位：度）
short int kincors232Fd133Driver::IpmTem()
{
    return m_DevTemp;
}

unsigned short int kincors232Fd133Driver::DrivStatus()
{
    return m_DevStautes;
}
...
void  kincors232Fd133Driver::signleRecord(message &msg)
{
    ...
    //设置成功
    if(msg.Get_sizetype()==0x60)            // It supposed to RBYTEE ???
    {
		m_ConfigFaild = true;
	}
    else if((msg.Get_sizetype()==RBYTE1)||(msg.Get_sizetype()==RBYTE2)||(msg.Get_sizetype()==RBYTE4))
    {
        ...
        switch(msg.Get_CMD())
        {
			...
			case GetVelocity:  //实时速度
                m_ActVelocity = (INT16)msg.get_data_m_param3();      // modified by ssj 5-18  <int->INT16, param34->param3>
                #if DEBUG_ECHO
			...
			case GetIniTCtrlMode:
				//工作模式
                m_ctrlmode = (char)msg.get_data_m_byte(5);      // It supposed to '4'
				#if DEBUG_ECHO
			...
			case GetCmdVelocity:
				//读取目标速度
                m_SetVelocity = (INT16)msg.get_data_m_param3();     // It supposed to <param34>  5-18
				#if DEBUG_ECHO
