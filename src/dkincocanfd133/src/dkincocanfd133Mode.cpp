...
kincocanfd133Mode::kincocanfd133Mode(std::string port, int *devadd, int *dir, int cnt)
:ttyport(port)
{
    for(int index=0;index<cnt;index++)
    {
        Fd133Driver[index].Set_add(devadd[index]);        // driver address [0, 1, 2, 3]
        Fd133Driver[index].set_dir(dir[index]);

        add[index] = devadd[index];                         // add[] is used for ROS msg communication. [pub and sub]
        velocity[index] =0;
        onlineMsg[index].online=FN_LOST;
...
{
    std::cout<< "hello kincocanfd133Mode_master "<<std::endl;
    ...
    {
        std::cout << "dkincocanfd133mode.cpp thread interrupt" << std:: endl;
    }
...
int kincocanfd133Mode::DriveInit(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
    ...
    DevSetMode(vecpa);          // add by ssj 4-17
    DevSetAVelocity(vecpa);
    DevSetDVelocity(vecpa);
...
// the parameter vecpa is not a vector, It had better modify, such as fd_socket_can
...
void kincocanfd133Mode::DevSetMode(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
    //发送
    static int recv_timeout=0;
    #if DEBUG
    std::cout<< "hello DevSetMode "<<std::endl;
    #endif

    for(int index=0;index<DRIVER_AMOUNT;index++)
    {
        vecpa[0]->clear();
        vecpa[0]->sendRequest(Fd133Driver[index].Cmd_SetMode( kincors232Fd133Driver::MODE_SPEED) );
        boost::this_thread::sleep(boost::posix_time::milliseconds(1));


        std::string errprint = ttyport+"  DevSetMode recv  error";
        int recvsize=DriverecvDelay(vecpa[0],RECV_DATA_SIZE,RECV_WAITE_TIME,errprint);
        if(recvsize==RECV_DATA_SIZE)
        {
            //存储数据
...
void kincocanfd133Mode::DevSetAVelocity(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
    static int recv_timeout=0;
    #if DEBUG
    std::cout<< "hello DevSetAVelocity "<<std::endl;
    #endif

    for(int index=0;index<DRIVER_AMOUNT;index++)
    {
        vecpa[0]->clear();
        vecpa[0]->sendRequest( Fd133Driver[index].Cmd_SetAVelocity(kincors232Fd133Driver::ACC_VELOCITY) );
        boost::this_thread::sleep(boost::posix_time::milliseconds(1));


        std::string errprint = ttyport+"  DevSetAVelocity recv  error";
        int recvsize=DriverecvDelay(vecpa[0],RECV_DATA_SIZE,RECV_WAITE_TIME,errprint);
        if(recvsize==RECV_DATA_SIZE)
        {
            //存储数据
...
void kincocanfd133Mode::DevSetDVelocity(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
    static int recv_timeout=0;
    #if DEBUG
    std::cout<< "hello DevSetDVelocity "<<std::endl;
    #endif

    for(int index=0;index<DRIVER_AMOUNT;index++)
    {
        vecpa[0]->clear();
        vecpa[0]->sendRequest( Fd133Driver[index].Cmd_SetDVelocity( kincors232Fd133Driver::DEC_VELOCITY) );
        boost::this_thread::sleep(boost::posix_time::milliseconds(1));


        std::string errprint = ttyport+"  DevSetDVelocity recv  error";
        int recvsize=DriverecvDelay(vecpa[0],RECV_DATA_SIZE,RECV_WAITE_TIME,errprint);
        if(recvsize==RECV_DATA_SIZE)
        {
            //存储数据
...
void kincocanfd133Mode::DevCtrlDisEndable(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
    ...
    {
		vecpa[0]->clear();
        vecpa[0]->sendRequest(Fd133Driver[index].Cmd_SetRelease());
		boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
int kincocanfd133Mode::DriveEmergencyStop(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
    ...
    DevEmergencyStop(vecpa);        // 0x6040 0b
...
void kincocanfd133Mode::dirv_pub_allstatus(int cnt)
{   
    std_msgs::String stdmsg;

    stdmsg.data = Pack_driver_status(cnt);

    rospub_mode_driverstatues.publish(stdmsg);
    ros::spinOnce();
}

std::string  kincocanfd133Mode::Pack_driver_status(int cnt)
{       
    // Verify that the version of the library that we linked against is
    // compatible with the version of the headers we compiled against.
    GOOGLE_PROTOBUF_VERIFY_VERSION;
