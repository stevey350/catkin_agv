...
public:
    ...
    enum DRIVER_MODE{
        ...
        MODE_SPEED =            0x03,
        MODE_TORQUE =           0x04,
        MODE_SPEED_INSTANT =    -3,
        MODE_MASTER_SLAVE =     -4,
        MODE_ORIGIN =           0x06,
        MODE_INTERPOLATION =    0x07,
    };

    enum{
        ACC_VELOCITY = 20,      // (rps/s)  1 rps/s = 60 rpm/s
        DEC_VELOCITY = 20,
    };
...
public :
    ...
    std::string Cmd_SetRelease();

    std::string Cmd_SetMode(DRIVER_MODE mode);
    std::string Cmd_SetAVelocity(UINT32 AVelocity);
    std::string Cmd_SetDVelocity(UINT32 DVelocity);
