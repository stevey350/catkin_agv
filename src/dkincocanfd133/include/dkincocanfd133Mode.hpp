...
public:
   ...
   kincocanfd133Mode();
   kincocanfd133Mode(std::string port,int *devadd,int *dir,int cnt);
    ...
    void DevSetMode(std::vector < boost::shared_ptr<socketcan> > &vecpa);           // add by ssj 4-17
    void DevSetAVelocity(std::vector < boost::shared_ptr<socketcan> > &vecpa);      // add by ssj
    void DevSetDVelocity(std::vector < boost::shared_ptr<socketcan> > &vecpa);      // add by ssj
    ...
    void dirv_pub_allstatus(int cnt);
    std::string  Pack_driver_status(int cnt);
