...
void AprilTagDetector::imageCb(const sensor_msgs::ImageConstPtr& msg, const sensor_msgs::CameraInfoConstPtr& cam_info)
{
    ...
    {       
        ...
        ROS_INFO("detectopm.id=%d", detection.id);
        ...
        AprilTagDescription description = description_itr->second;      // return AprilTagDescription
//        double tag_size = description.size();
        double tag_size = 0.028*320.0/659.0;
        ...
        //
        Eigen::Vector3d euler = rot.eulerAngles(0,1,2);  //rot_quaternion.eulerAngles(2,1,0); rot.eulerAngles(2,1,0)
        euler = euler/M_PI*180;
        ROS_INFO("x=%f, y=%f, z=%f.", tag_pose.pose.position.x, tag_pose.pose.position.y, tag_pose.pose.position.z);
//        ROS_INFO("1-roll=%f, pitch=%f, yaw=%f", euler[0], euler[1], euler[2]);


        Eigen::Matrix3d fixed_rot;
        fixed_rot = rot_quaternion.toRotationMatrix();
        ...
        roll = roll*180/M_PI;
        ROS_INFO("2-roll=%f, pitch=%f, yaw=%f", roll, pitch, yaw);

        if(yaw < 1e-2)
            yaw = yaw+360;
        yaw = 360-yaw;
        yaw = ((int)yaw % 360) + (yaw-(int)yaw);
        ROS_INFO("3-yaw=%f", yaw);
