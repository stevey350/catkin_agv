...
Dgv_Manage::Dgv_Manage()
{
    ...
    zmqcmdlist.insert(std::make_pair("HeartBeat",index++ ));    // HeartBeat <--> 1
    ...
    cmd_speedVelocity = 500;
    cmd_angelVelocity = 100;
    net_left = 500;
    net_right = 500;
...
void Dgv_Manage::Drivers_ros_cmdCall(const std_msgs::String::ConstPtr& msg)
{
    ...
    if(cmdstr.size()>20)
    {
        ...
        if(DriverInforecv.device_add() == DEV_DRIVER_ADD_R)
        {
            std::cout<<"DEV_DRIVER_ADD_R"<<std::endl;
			statues.Dirver[0].Clear();
			statues.Dirver[0].CopyFrom(DriverInforecv);
			statues.driverNode[0].Set_connect();
			statues.strDirver[0]=cmdstr;
		}
        else if(DriverInforecv.device_add() == DEV_DRIVER_ADD_L)
        {
            std::cout<<"DEV_DRIVER_ADD_L"<<std::endl;
...
{
    ros::Rate loop_rate(de_CTRLRATE);       // 20 Hz
    int satas_count = 0;
...
int Dgv_Manage::proc_zmq_recvcmd(std::string pack)
{
    ...
    int ret=0;
    std::string strto=msgrecv.to();     // The 'to' field is the destination which message want to send to.
    ...
    if(strto.compare( ZMQ_NAME_DRIVERCTRL) == 0)
    {
        ...
        int funid= (int)(it->second);       //obtain the second field value, i.e., <int> in map
        #if 1
        if(funid!=1)
        {
...
void Dgv_Manage::ros_LaserCtrl_SendRpc(std::string scmd,int leftrpm,int rightrpm)
{   
	//ros::NodeHandle npub;
	#if 0
	std::cout<<"scmd="<<scmd<<std::endl;
    ...
    if(scmd == "")
    {
        trans_4_velocity(leftrpm, rightrpm);
		//transSA_4_velocity(speed,angel);
		dgvmsg::DriverVelocity Motec;
        Motec.driver.resize(2);
		...
		Motec.driver[0].name="DEV_DIR_ADD_LF";
        Motec.driver[0].add = DEV_DRIVER_ADD_R;
		Motec.driver[0].VRPM = velocity[0];

		Motec.driver[1].name="DEV_DIR_ADD_LB";
        Motec.driver[1].add = DEV_DRIVER_ADD_L;
		Motec.driver[1].VRPM = velocity[1];
...
void Dgv_Manage::trans_4_velocity( float leftrpm, float rightrpm)
{
...
void Dgv_Manage::transSA_4_velocity( float speed, float angel)
{
    ...
    // RIGHT
    velocity[0]=calculate_rpm_speed(DEV_DRIVER_ADD_R, rightspeed);
    ...
    //LEFT
    velocity[1]=calculate_rpm_speed(DEV_DRIVER_ADD_L,leftspeed);
...
int Dgv_Manage::calculate_rpm_speed(char dev_add, float speed)
{
    ...
    if(dev_add == DEV_DRIVER_ADD_R)
    {
        //R
        rpm = (float)( (speed/(1000.0*deR_WheelSize))*(de_ReductionRatiol*60) );    // m/s -> rpm
    }
    else if(dev_add == DEV_DRIVER_ADD_L)
    {
        //L
        rpm = (float)( (speed/(1000.0*deL_WheelSize))*(de_ReductionRatiol*60) );
    }
//    else if(dev_add ==DEV_DIR_ADD_RF)
//    {
//	  	//RF
//        rpm = (float)( (speed/(1000.0*deRF_WheelSsize))*(de_ReductionRatiol*60) );
//    }
//    else if(dev_add ==DEV_DIR_ADD_RB)
//    {
//	  	//RB
//        rpm = (float)( (speed/(1000.0*deRB_WheelSsize))*(de_ReductionRatiol*60) );
//    }
...
// 1-obtain the value of 'cmd_speedVelocity'
// 2-send the return message to center.
int  Dgv_Manage::Zmq_Recv_CAR_FORWARD(PackageMessage &MsgRECV)
{
    ...
    if(size == 1)
    {
        ...
        try
        {
            recvspeed = boost::lexical_cast<int>(pcallv->parameters(0)); //
        }
        catch(boost::bad_lexical_cast&)    //
        {
            recvspeed=0;
        }
        cmd_speedVelocity = recvspeed>de_CTRLMAXSPEED?de_CTRLMAXSPEED:(recvspeed<0?0:recvspeed); // 0~de_CTRLMAXSPEED
        Result->set_errorcode(0);
    }
...
// set the value of cmd_angelVelocity
...
// set the values of cmd_speedVelocity and cmd_angelVelocity as 0
...
int  Dgv_Manage::Zmq_Recv_SPEED_ANGLE(PackageMessage &MsgRECV)
{
    ...
    if(size == 2)
    {
        ...
        try
        {
            recvth=boost::lexical_cast<int>(pcallv->parameters(1));
        }
        catch(boost::bad_lexical_cast&)
        {
            recvth=0;
...
void Dgv_Manage::Laser_Callback(const std_msgs::String& laser)
{
    std::cout<<"laser = "<<laser.data<<std::endl;
    if(laser.data == "111")
       laser_flag = 1;
    else
       laser_flag = 0;
...
void Dgv_Manage::Cmd_Vel_Callback(const geometry_msgs::Twist& cmd_vel)
{
    float speed_velocity = 0.0;
    ...
    {   
	    ...
	    speed_velocity = 0.0;
	...
	else
         {
          ...
          speed_velocity = cmd_vel.linear.x;
          angle_velocity = cmd_vel.angular.z;
